...
export async function POST(request: NextRequest) {
  try {
    ...
    // Build phases
    Array.from(phaseMap.keys()).sort((a, b) => a - b).forEach((phaseNum, index) => {
      ...
      const phaseStartDay = Math.min(...phaseItems.map(item => taskDays.get(item.id)?.start || currentDay))
      const phaseEndDay = Math.max(...phaseItems.map(item => taskDays.get(item.id)?.end || currentDay))
